...
export default function HexBoard({
  ...
  changerMap = {},
  bombMap = new Set(),
}: HexBoardProps): React.JSX.Element {
    ...
    >
      ...
      <g aria-hidden="true">
        {positioned.map(({ node, x, y }) => {
          ...
          const rot = ARROW_ROTATION[changerDir];
          return (
      ...
      <g aria-hidden="true">
        {positioned.map(({ node, x, y }) => {
          if (!bombMap.has(node.id)) {
            ...
            if (
              !anim ||
              (anim.state !== "waiting" && anim.state !== "exploding")
            ) {
              return null;
            }
          }
          ...
          if (anim?.state === "exploding" || anim?.state === "gone") {
            return null;
          }
          return (
      ...
      {positioned.map(({ node, x, y }) => {
        const anim = animStates.get(node.id);
        if (activeIds && !activeIds.has(node.id) && !anim) {
          return null;
        }
        return (
          <HexNode
