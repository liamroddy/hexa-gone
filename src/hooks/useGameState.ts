...
export function useGameState(radius = 2, hexSize = 30): GameState {
  ...
  const initialArrows = useMemo(
    () =>
      new Map<string, Direction | undefined>(
        board.playableNodes.map((n) => [n.id, n.arrowDirection]),
      ),
    [board.playableNodes],
  );
  ...
  useEffect(() => {
    ...
    setAnimCount(0);
  }, [board.playableNodes, board.bombMap, board.minMoves]);
  ...
  const retry = useCallback(() => {
    ...
    animatingIds.current = new Set();
    setAnimCount(0);
  }, [board.playableNodes, board.bombMap, board.minMoves, initialArrows]);

  const handleHexClick = useCallback(
    (node: HexNodeData) => {
      if (animatingIds.current.has(node.id)) {
        return;
      }
