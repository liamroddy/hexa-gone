...
function placeBombs(
  ...
  for (let i = 0; i < earlyThreshold; i++) {
    earlyPeeled.add(solveOrder[i]!.id)
  }
  ...
  for (let si = totalSteps - 1; si >= Math.floor(totalSteps * 0.4); si--) {
    ...
    const escapePath = traceEscapePath(step.id, step.dir, step.remainingSnapshot, nodeMap, changerMap)

    for (const hexId of escapePath) {
      // The bomb hex must have been an early-peeled playable tile
      if (!earlyPeeled.has(hexId)) continue
      if (changerPositions.has(hexId)) continue
      if (bombMap.has(hexId)) continue

      const bombNode = nodeMap[hexId]
      if (!bombNode) continue
      ...
      for (const dir of ALL_DIRECTIONS) {
        ...
        if (neighborArrow && neighborArrow === oppositeDir(dir)) {
          hasAdjacentPointer = true
          break
        }
      }
      if (hasAdjacentPointer) continue
      ...
      for (const dir of ALL_DIRECTIONS) {
        const neighborId = bombNode.neighbors[dir]
        if (neighborId && step.remainingSnapshot.has(neighborId) && neighborId !== step.id) {
  ...
  for (const cand of candidates) {
    if (bombMap.size >= desiredCount) break
    if (usedTriggers.has(cand.triggerIndex)) continue
    if (usedBombHexes.has(cand.bombHexId)) continue
...
function traceEscapePath(
  ...
  while (currentId !== null) {
    ...
    if (remaining.has(currentId) && currentId !== nodeId) return path
    path.push(currentId)
